...
{
    /// <summary>Bse_User_Role表数据访问类
    ///
    /// </summary>
    public partial class Bse_User_RoleDAL
    {
        public Bse_User_RoleDAL()
        { }

        /// <summary>增加一条数据
        ///
        /// </summary>
        public int Add(Nikita.Permission.Model.Bse_User_Role model)
        {
            IMsSqlDataAccessService h = ServiceHelper.GetMsSqlDataAccessService();
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Bse_User_Role(");
            strSql.Append("UserId, RoleId, PY, Sort, Dept_Id, DeptName, Company_Id, CompanyName, Bloc_Id, BlocName, CreateUserId, CreateName, CreateDate, EditUserId, EditUserName, EditDate, Remark, State, SystemId  )");
            ...
            {
                h.AddParameter("@UserId", DBNull.Value);
            }
            else
            {
                h.AddParameter("@UserId", model.UserId);
            }
            if (model.RoleId == null)
            {
                h.AddParameter("@RoleId", DBNull.Value);
            }
            else
            {
                h.AddParameter("@RoleId", model.RoleId);
            }
            if (model.PY == null)
            {
        ...
        {
            IMsSqlDataAccessService h = ServiceHelper.GetMsSqlDataAccessService();
            string sql = "select count(1) from Bse_User_Role";
        ...
        ///
        /// </summary>
        public bool Delete(int User_Role_Id)
        {
            IMsSqlDataAccessService h = ServiceHelper.GetMsSqlDataAccessService();
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from Bse_User_Role ");
            strSql.Append(" where User_Role_Id=@User_Role_Id ");

            h.CreateCommand(strSql.ToString());
            h.AddParameter("@User_Role_Id", User_Role_Id);
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(" FROM Bse_User_Role ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetList(string fileds, string order, string ordertype, int PageSize, int PageIndex, string strWhere)
        {
            ...
            h.AddParameter("@tblName", "Bse_User_Role");
        ...
        ///
        /// </summary>
        public bool Update(Nikita.Permission.Model.Bse_User_Role model)
        {
            IMsSqlDataAccessService h = ServiceHelper.GetMsSqlDataAccessService();
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Bse_User_Role set ");
            strSql.Append("UserId=@UserId, RoleId=@RoleId, PY=@PY, Sort=@Sort, Dept_Id=@Dept_Id, DeptName=@DeptName, Company_Id=@Company_Id, CompanyName=@CompanyName, Bloc_Id=@Bloc_Id, BlocName=@BlocName, CreateUserId=@CreateUserId, CreateName=@CreateName, CreateDate=@CreateDate, EditUserId=@EditUserId, EditUserName=@EditUserName, EditDate=@EditDate, Remark=@Remark, State=@State, SystemId=@SystemId  ");
            strSql.Append(" where User_Role_Id=@User_Role_Id ");

            h.CreateCommand(strSql.ToString());
            if (model.User_Role_Id == null)
            {
                h.AddParameter("@User_Role_Id", DBNull.Value);
            }
            else
            {
                h.AddParameter("@User_Role_Id", model.User_Role_Id);
            }
            if (model.UserId == null)
            {
                h.AddParameter("@UserId", DBNull.Value);
