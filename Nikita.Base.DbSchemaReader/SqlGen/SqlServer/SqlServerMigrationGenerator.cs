...
namespace Nikita.Base.DbSchemaReader.SqlGen.SqlServer
{
    class SqlServerMigrationGenerator : MigrationGenerator
    {
        ...
        public override string RenameTable(string newTableName, string originalTableName)
        {
            if (string.IsNullOrEmpty(originalTableName) || string.IsNullOrEmpty(newTableName))
            {
                return string.Empty;
            }
            else
            {
                return "sp_rename '" + originalTableName + "', '" + newTableName + "';";
