...
{
    public class SqlServerEntityFrameworkHelper : IBseDbHelper
    { 
        string strConn = "";
        public SqlServerEntityFrameworkHelper(string connString)
        {
            strConn = connString;
        }
        ...
        #region ExfSoft.EntityFramework.Extension扩展分页
        /// <summary>获取分页集合 
        /// 获取分页集合 
        /// </summary>
        /// <returns></returns>
        public PagerResultModel<T> GetListPaged<T, Tkey>(IQueryable<T> TSource, int Pagesize, int Pageindex) where T : class
        {
            PagerResultModel<T> PagerResultModels = PagerExtension.ToPager<T>(TSource, Pagesize, Pageindex);
            return PagerResultModels;
        }
        /// <summary>异步获取分页集合 
        /// 异步获取分页集合 
        /// </summary>
        /// <returns></returns>
        public Task<PagerResultModel<T>> GetListPagedAsync<T, Tkey>(IQueryable<T> TSource, int Pagesize, int Pageindex) where T : class
        {
            Task<PagerResultModel<T>> PagerResultModels = PagerExtension.ToPagerAsync<T>(TSource, Pagesize, Pageindex);
            return PagerResultModels;
        }
        #endregion

        #region EntityFramework.Extension
        /// <summary>批量删除
        /// 批量删除
        /// </summary>
        /// <typeparam name="T"></typeparam>
