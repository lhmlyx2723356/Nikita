...
namespace Nikita.Assist.WcfService
{
    ...
    public class MssqlHelper
    {
        ...
        {
            string connStr =
                ConfigurationManager.ConnectionStrings["MyFrameConnectionString"].ToString();
        ...
        public void ExecuteBulkToDb(DataTable dt, string strTableName)
        {
            if (_conn.State == ConnectionState.Closed)
            {
                _conn.Open();
            }
            SqlBulkCopy bulkCopy = new SqlBulkCopy(_conn) { DestinationTableName = strTableName, BatchSize = dt.Rows.Count };
            try
            {
                if (dt.Rows.Count != 0)
