/// <summary>说明:FrmCacheSettingSimpleQuery文件
/// 作者:卢华明
...
{
    /// <summary>说明:FrmCacheSettingSimpleQuery
    /// 作者:Luhm
    /// 最后修改人:
    /// 最后修改时间:
    /// 创建时间:2016/6/30 21:55:58
    /// </summary>
    public partial class FrmCacheSettingSimpleQuery : Form
    {
        #region 常量、变量
  /// <summary>DataGridView下拉框绑定数据源
        ...
        ///
        /// </summary>
        public FrmCacheSettingSimpleQuery()
        {
            InitializeComponent(); 
 ...
 m_CacheSettingDAL = GlobalHelp.GetResolve<IBseDAL<CacheSetting>>();
  ...
  private void grdData_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
    ...
    try
        ...
        m_lstCacheSetting =   m_CacheSettingDAL.GetListArray("*", "Id", "ASC", Pager.PageSize, Pager.PageIndex, strWhere);
            Pager.RecordCount = m_CacheSettingDAL.CalcCount(strWhere);
            Pager.InitPageInfo();
...
condition.AddCondition("SetText", this.txtQuerySetText.Text, SqlOperator.Like);
        ...
        private void DoDeleteOrCancel(string strOperation)
        {
            ...
            var blnReturn = strOperation.Trim() == "删除" ?   m_CacheSettingDAL.DeleteByCond(" Id in (" + strIds + ")") :    m_CacheSettingDAL.Update("Status =0", " Id in (" + strIds + ")");
            if (blnReturn)
            {
