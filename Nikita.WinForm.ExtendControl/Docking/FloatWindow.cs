    ...
    public class FloatWindow : Form, INestedPanesContainer, IDockDragSource
    {
        ...
        internal void TestDrop(IDockDragSource dragSource, DockOutlineBase dockOutline)
        {
            if (VisibleNestedPanes.Count == 1)
            {
                ...
                {
                    if (NativeMethod.SendMessage(Handle, (int)Win32.Msgs.WM_NCHITTEST, 0, lParam) == (uint)Win32.HitTest.HTCAPTION)
                    {
                ...
                case (int)Win32.Msgs.WM_NCLBUTTONDOWN:
                    {
                        ...
                        uint result = Win32Helper.IsRunningOnMono ? 0 : NativeMethod.SendMessage(this.Handle, (int)Win32.Msgs.WM_NCHITTEST, 0, (uint)m.LParam);
                ...
                case (int)Win32.Msgs.WM_NCLBUTTONDBLCLK:
                    {
                        uint result = !DoubleClickTitleBarToDock || Win32Helper.IsRunningOnMono
                            ? 0
                            : NativeMethod.SendMessage(this.Handle, (int)Win32.Msgs.WM_NCHITTEST, 0, (uint)m.LParam);
