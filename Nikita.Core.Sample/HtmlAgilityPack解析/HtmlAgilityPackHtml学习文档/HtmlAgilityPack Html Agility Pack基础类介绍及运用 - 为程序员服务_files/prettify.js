...
//saveCode
function saveCode(code,ext) {
    var ie = /MSIE/.test(navigator.userAgent);
    if (ie){
        var winname = window.open('', '_blank', 'height=1,width=1,top=200,left=300');
        winname.document.open('text/html', 'replace');
        winname.document.writeln(code);
        winname.document.execCommand('saveas','','code' + ext);
        winname.close();
    }else{
        if($('#formSaveCode').length==0){
            $('<form method="post" target="_blank" action="http://'+host+'/code-snippet/saveCode" id="formSaveCode">'
                +'<input type="hidden" name="ext" id="saveCodeExt"/>'
                +'<input type="hidden" name="code" id="saveCodeCode"/>'
                +'</form>').appendTo($(document.body));
        }
        $('#saveCodeExt').val(ext);
        $('#saveCodeCode').val(code);
        $('#formSaveCode').submit();
    }
}
var langExt = {
...
//use in oom
ZeroClipboard.setMoviePath( '/static/clipboard/ZeroClipboard10.swf');
window.bindRunCodeEvt = false;
window.bindSaveCodeEvt = false;
$(function(){
    $('pre').hover(function(e){
        ...
        var code = $(pre).text().replace(/(?!\r)\n/g,'\r\n');
        var cssClass = $(pre).attr('lang') || $(pre).attr('class') ;
        var codeLang = (typeof cssClass != 'undefined' && cssClass && cssClass.length > 'prettyprint '.length)
            ? cssClass.replace('prettyprint','').replace('prettyprinted','').trim().toLowerCase()
            : (cssClass || 'txt');
        ...
        if(codeTools.length==0){
            codeTools = $('<div class="codeTools"><div id="copyCode">复制代码</div><div id="saveCode">保存代码</div><div id="runCode">试试效果</div></div>').appendTo(document.body);
        }

        var oRunCode = $('#runCode');
        if(codeLang == 'html' || codeLang == 'htm' || codeLang == 'tryit'
            || (codeLang == 'javascript' && code.toLowerCase().indexOf('<html>')>-1)) {
            oRunCode.css('display','block');
        }else{
            oRunCode.css('display','none');
        }
		window.runCode = code;
        if(!window.bindRunCodeEvt) {
            oRunCode.click(function(ev){
                if($('#formRunCode').length==0){
                    $('<form method="post" target="_blank" action="http://'+host+'/code-snippet/runCode" id="formRunCode">'
                        +'<input type="hidden" name="code" id="runCodeCode"/>'
                        +'</form>').appendTo($(document.body));
                }

                runCode = window.runCode.replace(/<(\/?)script([^>]*)>/ig,'<$1oomRunJs$2>');

                $('#runCodeCode').val(runCode);
                $('#formRunCode').submit();
            });
            window.bindRunCodeEvt=true;
        }

        var oSaveCode = $('#saveCode');
        window.oomSaveCode = code;
        window.codeLang = codeLang;
        if(!window.bindSaveCodeEvt ){
            oSaveCode.click(function(ev){
                var ext = '.html';
                var lang = window.codeLang.toLowerCase();
                if(langExt[lang]) ext = '.'+langExt[lang];
                else {
                    if(lang) ext = '.'+lang;
                }
                saveCode(window.oomSaveCode,ext);
            });
            window.bindSaveCodeEvt =true;
        }

        codeTools.css({
            'left': (position.left+$(pre).width()-codeTools.width()-10) + 'px',
            'top': (position.top + 14 + ($('.wrap').css('position') == 'relative' ? $('.wrap').position().top:0)) + 'px'
        });
        var clip;
        if(!window.codeClipboard){
            window.codeClipboard = new ZeroClipboard.Client();
            clip = window.codeClipboard;
            clip.addEventListener( 'onComplete', function(client, text) {
                alert("已复制代码");
            } );
            clip.addEventListener( 'onMouseDown', function(client) {
                clip.setText(clip.code);
            } );
            clip.glue( 'copyCode');
        }else{
            clip = window.codeClipboard;
