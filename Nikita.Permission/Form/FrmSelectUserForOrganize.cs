...
{
    public partial class FrmSelectUserForOrganize : FrmBase
    {
        ...
        private readonly Bse_User_OrganizeDALExtend _userOrganizeDalExtend = new Bse_User_OrganizeDALExtend();

        private string _strUserIds;

        private Bse_User_OrganizeDAL _userRoleDal = new Bse_User_OrganizeDAL();

        public FrmSelectUserForOrganize()
        {
            InitializeComponent();
        }

        public FrmSelectUserForOrganize(string roleId)
        {
            InitializeComponent();
            _roleId = roleId;
        }
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            bool flag = _userOrganizeDalExtend.AddUserToOrganize(DxCtlHelper.GetSelectRowsFileValue(gridVMain, "User_Id", ','), _roleId, UserInfoHelper.CreateUserId,
        ...
        private void DoBind()
        {
            ...
            DataTable dtUserName = _userOrganizeDalExtend.GetUserByOrganizeId("a.Organize_Id=" + _roleId + " and a.SystemId=" + UserInfoHelper.SystemId + "").Tables[0];
            if (DataTableHelper.IsHaveRows(dtUserName))
            {
                memoEdit1.Text = DataTableHelper.GetStringWithSplit(dtUserName, "Realname", ',');
                _strUserIds = DataTableHelper.GetStringWithSplit(dtUserName, "User_Id", ',');
            }
            else
            {
                _strUserIds = string.Empty;
            }
        }
        ...
        private void treDept_FocusedNodeChanged(object sender, FocusedNodeChangedEventArgs e)
        {
            if (e.Node != null)
            {
                ...
                //过滤已经存在的用户
                if (_strUserIds.Length > 0)
                {
                    DataRow[] drs = dtFocusDeptUser.Select(" User_Id not in (" + _strUserIds + ") ");
                    if (drs.Length > 0)
                    {
        ...
        private void treeList2_FocusedNodeChanged(object sender, FocusedNodeChangedEventArgs e)
        {
            if (e.Node != null)
            {
                ...
                DataTable dtFocusRoleUser = _userOrganizeDalExtend.GetUserByOrganizeId("a.Organize_Id=" + roleId + " and a.SystemId=" + UserInfoHelper.SystemId + "").Tables[0];
                //过滤已经存在的用户
                if (_strUserIds.Length > 0)
                {
                    DataRow[] drs = dtFocusRoleUser.Select(" User_Id not in (" + _strUserIds + ") ");
                    if (drs.Length > 0)
                    {
                        gridCMain.DataSource = drs.CopyToDataTable();
                    }
