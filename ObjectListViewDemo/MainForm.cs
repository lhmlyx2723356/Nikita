...
namespace ObjectListViewDemo {
    ...
    public partial class MainForm {
        ...
        List<Person> _masterList;
        void InitializeExamples() {
            // Use different font under Vista
            if (ObjectListView.IsVistaOrLater)
                this.Font = new Font("Segoe UI", 9);

            _masterList = new List<Person>
            {
                new Person("Wilhelm Frat", "Gymnast", 21, new DateTime(1984, 9, 23), 45.67, false, "ak",
                    "Aggressive, belligerent "),
                new Person("Alana Roderick", "Gymnast", 21, new DateTime(1974, 9, 23), 245.67, false, "gp",
                    "Beautiful, exquisite"),
                new Person("Frank Price", "Dancer", 30, new DateTime(1965, 11, 1), 75.5, false, "ns",
                    "Competitive, spirited"),
                new Person("Eric", "Half-a-bee", 1, new DateTime(1966, 10, 12), 12.25, true, "cp",
                    "Diminutive, vertically challenged"),
                new Person("Nicola Scotts", "Nurse", 42, new DateTime(1965, 10, 29), 1245.7, false, "np",
                    "Wise, fun, lovely"),
                new Person("Madalene Alright", "School Teacher", 21, new DateTime(1964, 9, 23), 145.67, false, "jr",
                    "Extensive, dimensionally challenged"),
                new Person("Ned Peirce", "School Teacher", 21, new DateTime(1960, 1, 23), 145.67, false, "gab",
                    "Fulsome, effusive"),
                new Person("Felicity Brown", "Economist", 30, new DateTime(1975, 1, 12), 175.5, false, "sp",
                    "Gifted, exceptional"),
                new Person("Urny Unmin", "Economist", 41, new DateTime(1956, 9, 24), 212.25, true, "cr",
                    "Heinous, aesthetically challenged"),
                new Person("Terrance Darby", "Singer", 35, new DateTime(1970, 9, 29), 1145, false, "mb",
                    "Introverted, relationally challenged"),
                new Person("Phillip Nottingham", "Programmer", 27, new DateTime(1974, 8, 28), 245.7, false, "sj",
                    "Jocular, gregarious"),
                new Person("Mister Null")
            };

            List<Person> list = new List<Person>();
            foreach (Person p in _masterList)
...
//            this.olvComplex.DisabledItemStyle.Font = new Font("Stencil", 10);
            ...
            */
            ...
            this.personColumn.ImageGetter = delegate(object row) {
                ...
                if (name.Length > 0 && "AEIOU".Contains(name.Substring(0, 1)))
                    return "star";
                if (String.Compare(name, "T", StringComparison.Ordinal) < 0)
                    return 2; // person
        ...
        void InitializeVirtualListExample() {
            ...
            this.olvVirtual.VirtualListDataSource = new ExampleVirtualDataSource(this.olvVirtual, this._masterList);
            ...
            this.olvVirtual.CustomSorter = delegate(OLVColumn col, SortOrder order) {
                _masterList.Sort(new MasterListSorter(col, order));
            };
        ...
        private void InitializeFastListExample(List<Person> list) {
            ...
            this.olvColumn18.ImageGetter = delegate(object row) {
                ...
                if ("AEIOU".Contains(person.Name.Substring(0, 1)))
                    return 0; // star
                                                                    if (String.Compare(person.Name, "N", StringComparison.Ordinal) < 0)
                                                                        return 1; // heart
                                                                    else
                                                                        return 2; // music
            };
        ...
        private void button14_Click(object sender, EventArgs e) {
            ...
            while (l.Count < 1000)
                foreach (Person x in this._masterList)
                    l.Add(new Person(x));
